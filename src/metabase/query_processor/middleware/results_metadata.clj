...
(def ^:private DateTimeUnitKeywordOrString
  ...
  (s/constrained su/KeywordOrString
                 (fn [unit]
                   (contains? qpi/datetime-field-units (keyword unit)))
...
(s/defn ^:private ^:always-validate results->column-metadata :- (s/maybe ResultsMetadata)
  ...
  (seq (for [col (:cols results)]
         ;; if base-type isn't set put a default one in there. Similarly just use the value of `:name` for `:display_name` if one isn't set
         (merge {:base_type    :type/*
                 :display_name (:name col)}
